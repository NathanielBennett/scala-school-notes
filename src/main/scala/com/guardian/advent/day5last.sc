...
def makeNumberList(numbers: String): Option[List[List[Long]]] = Try {

  def tupleLoop(l: List[Long], maybeStart: Option[Long] = None, acc: List[NumericRange.Exclusive[Long]] = List.empty ):  List[NumericRange.Exclusive[Long]] =
    l match {
      case Nil => acc
      case head :: tail =>
        maybeStart match {
  ...
  tupleLoop(longList).map { range => List.range(range.start, range.end) }
}.toOption

val seedsR = """seeds:\s+(.+)""".r

def parseSeeds(line: String): Option[List[List[Long]]] =  for {
  seedNumbers <- Try {
...
val l = parseSeeds(lines.head).map { seeds =>
  seeds.map { seed => seed.map { s => (s, findValueForSeed(s, seedMaps)) } // seed => se seed.map(findValueForSeed(seed, seedMaps) }
  }
}
