...
val t = findStart(tiles).map {
  start =>
    val (actualStart, firstTiles) = start.startTiles(tiles)0
    ...
    val vertices = loopTiles.filter(t => verticePipes.contains(t.pipe))

    nonLoopTiles.filter{ t => t.isEnclosed(allTiles, vertices) }.size
}
